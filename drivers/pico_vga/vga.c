...
static void __time_critical_func(dmaIrqHandler)(void)
{
  ...
  if (dma_hw->ints0 & (1u << syncDmaChan))
  {
    ...
    if (vgaParams.endOfScanlineFn) vgaParams.endOfScanlineFn();
  }
  ...
  if (dma_hw->ints0 & (1u << rgbDmaChan))
  {
    ...
    // CRITICAL: do NOT call divmod_u32u32 here - it's a SIO-accelerated
    // divider helper that lives in flash (0x10003e40). When core 0 is
    // mid-flash_range_erase the entire flash is unreadable, and core 1
    // calling out to a flash-resident veneer here would hard-fault and
    // VGA would die. Inline the divmod for the vPixelScale we actually
    // use (always 2 in this build per vga_frens.c overrides). The fast
    // path stays entirely in this RAM-resident IRQ handler.
    uint32_t curr = currentDisplayLine++;
    uint32_t vScale = vgaParams.params.vPixelScale;
    uint32_t pxLine, pxLineRpt;
    if (vScale == 2) { pxLine = curr >> 1; pxLineRpt = curr & 1; }
    else if (vScale == 1) { pxLine = curr; pxLineRpt = 0; }
    else if (vScale == 4) { pxLine = curr >> 2; pxLineRpt = curr & 3; }
    else if (vScale == 3) {
        // Visrealm uses 3 in HBC-56 firmware; do iterative subtract-3
        // (max 8 iterations for our 600-line frames) instead of divmod.
        pxLine = 0; pxLineRpt = curr;
        while (pxLineRpt >= 3) { pxLineRpt -= 3; ++pxLine; }
    }
    else { pxLine = 0; pxLineRpt = 0; /* unsupported vPixelScale */ }
    ...
    if ((pxLineRpt == 0))
    {
      ...
      // Render the next scanline directly here (no multicore FIFO).
      // scanline_cb is in RAM (.time_critical.scanline_cb) so this is
      // safe even when core 0 has interrupts disabled and is in the
      // middle of a flash erase. The buffer used for output is the
      // OPPOSITE parity of pxLine because rgb DMA is currently scanning
      // out the buffer for `pxLine`; the next line will be (pxLine+1)
      // which has opposite parity (assuming vPixelScale != 0 and the
      // standard alternating Even/Odd setup the original visrealm code
      // implements).
      uint16_t *targetBuf = (requestLine & 1) ? rgbDataBufferOdd : rgbDataBufferEven;
      if (vgaParams.scanlineFn) vgaParams.scanlineFn((uint16_t)requestLine, &vgaParams.params, targetBuf);

      if (requestLine == vgaParams.params.vVirtualPixels - 1)
      {
        if (vgaParams.endOfFrameFn) vgaParams.endOfFrameFn(s_frameNumber++);
      }
    }
  }
}
 ...
 *
 * Note: irq_set_exclusive_handler / irq_set_enabled are deferred to
 * vgaScanloop, which runs on core 1. Setting the IRQ on core 1 means
 * that core 0 can disable its own interrupts (e.g. during flashrom's
 * flash_range_erase / flash_range_program window) without ever
 * stalling the sync DMA chain or starving the PIO TX FIFO.
 */
static void initDma()
{
 ...
 * main vga loop
 *
 * Originally this was static and launched on core1 by vgaInit().
 * For the pico-infonesPlus port, the emulator owns multicore_launch_core1(),
 * so vgaInit() no longer launches it. The caller must invoke vgaScanloop()
 * (e.g. from its core1 entry) to service per-scanline callbacks.
 */
// vgaScanloop now just installs the DMA IRQ on core 1 and parks. The
// IRQ handler (dmaIrqHandler) does ALL the work: it restarts the sync
// and rgb DMA chains, calls scanline_cb directly, and calls
// endOfFrameFn/endOfScanlineFn directly. No multicore FIFO needed.
//
// Why the IRQ runs on core 1 (not core 0): during flashrom, core 0
// disables its own interrupts for ~150 ms per flash erase block. If
// the DMA IRQ were on core 0, the sync DMA chain would stall during
// each erase and the monitor would lose lock repeatedly. Core 1's
// interrupts are unaffected by what core 0 does, so the chain stays
// alive even through long flash operations.
//
// Placed in .time_critical so the function body is in RAM. WFE puts
// core 1 to sleep until the next IRQ, then resumes WFE. No flash
// fetch in the steady-state loop.
__attribute__((section(".time_critical.vgaScanloop"))) void vgaScanloop()
{
  // Install the DMA IRQ handler on this (core 1) core's vector table
  // and enable the IRQ. The handler in vga.c is __time_critical_func
  // so it lives in RAM and is safe to run while core 0 is mid-flash-op.
 ...
 * initialise the vga
 *
 * Caller is responsible for invoking vgaScanloop() afterwards on the core
 * where scanline callbacks should run (typically core1).
 */
