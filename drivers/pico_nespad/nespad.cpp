...
#include "nespad.h"  /* pulls in extern "C" linkage for the public API */

#define nespad_wrap_target 0

#if HW_CONFIG == 12 || HW_CONFIG == 13 || HW_CONFIG == 15 // Murmulator M1/M2 + PICO-56: both pads share latch and clock
...
/* Non-blocking variant: returns true and writes to *out if RX FIFO has
 * a sample ready; returns false (and leaves *out untouched) otherwise.
 * Lets the caller distinguish "PIO not producing data" from "all bits 0". */
extern "C" bool nespad_read_try(uint8_t pad, uint8_t *out)
{
    if (pad >= 2 || sm[pad] < 0 || !out) return false;
    if (pio_sm_is_rx_fifo_empty(pio[pad], sm[pad])) return false;
    *out = (uint8_t)((pio_sm_get(pio[pad], sm[pad]) >> 24) ^ 0xFF);
    return true;
}
